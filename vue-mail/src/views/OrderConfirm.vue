      ...
      <div class="checkout-order">
        ...
        <div class="order-foot-wrap">
          ...
          <div class="next-btn-wrap">
            <button v-on:click="payMent" class="btn btn--m btn--red">Proceed to payment</button>
          </div>
        </div>
      </div>
    </div>
...
//import {currency} from "./../util/currency"  //app.js已定义全局过滤器 可直接使用
  ...
  export default{
      ...
      methods:{
        ...
        payMent() {
          var addressId = this.$route.query.addressId; //从路由对象里拿到 注意是$route
          ...
          axios.post("/users/payMent",{
            ...
            let res = response.data;
            if (res.status == '0'){
              console.log("order created succeed");
              this.$router.push('/orderSuccess?orderId=' + res.result.orderId); //订单创建完毕跳转 注意时$router
            }
          });
        }
      }
