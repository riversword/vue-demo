<template>
    <div>
        ...
        <div class="container">
          <div class="checkout-addr">
            ...
            <div class="addr-list-wrap">
              <div class="addr-list">
                <ul>
                  <li v-for="(item, index) in addressListFilter" v-bind:class="{'check': checkIndex == index}" v-on:click="checkIndex = index;  selectedAddrId = item.addressId">
                    <dl>
                      <dt>{{item.userName}}</dt>
                    ...
                    <div class="addr-opration addr-del">
                      <!-- click.stop阻止click事件冒泡到li元素上 -->
                      <a v-on:click.stop="delAddressConfirm(item.addressId)" href="javascript:;" class="addr-del-btn">
                        <svg class="icon icon-del"><use xlink:href="#icon-del"></use></svg>
                      </a>
                    </div>
                    <div class="addr-opration addr-set-default">
                      <a v-on:click="setDefault(item.addressId)" v-if="!item.isDefault" href="javascript:;" class="addr-set-default-btn"><i>Set default</i></a>
                    </div>
              ...
              <div class="shipping-addr-more">
                <a v-on:click="expand" v-bind:class="{'open': limit>3}" class="addr-more-btn up-down-btn" href="javascript:;">
                  more
                  <i class="i-up-down">
      ...
      <modal v-bind:mdShow="isModalShow" v-on:close="closeModal">
        <p slot="message">确认删除此地址吗？</p>
        <div slot="btnGroup">
          <a v-on:click="delAddress" class="btn btn--m" href="javascript:;">确定</a>
          <a v-on:click="isModalShow = false" class="btn btn--m" href="javascript:;">取消</a>
        </div>
      </modal>
  ...
  export default{
      data(){
          return{
            addressList: [],
            limit: 3,
            checkIndex: null, //默认选中第几条
            isModalShow: false, //模态框显示/隐藏
            delAddressId: '', //待删除的addressId
            selectedAddrId: ''
          }
      },
      mounted(){
          ...
          axios.get("/users/addressList").then((response)=>{
            ...
            for (var i=0; i<res.result.length; i++) {
              if (res.result[i].isDefault) {
                this.checkIndex = i;
                this.selectedAddrId = res.result[i].addressId;
                break;
              }
            }
